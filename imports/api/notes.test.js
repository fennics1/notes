...
if (Meteor.isServer) {
  describe('note', function () {
    ...
    it('should update notes', function() {
      const title = 'This title for test'
      Meteor.server.method_handlers['notes.update'].apply(
        {userId:noteOne.userId}, [noteOne._id, { title }]
      );
    ...
    it('should throw error if extra updates provided', function() {
      expect(()=>{
        Meteor.server.method_handlers['notes.update'].apply(
          ...
          [ noteOne._id, { title:"new title", name:"extra name" }]
        );
      }).toThrow();
    });

    it('should not update if user was not creator', function() {
      const title = 'Update title from random guy'
      Meteor.server.method_handlers['notes.update'].apply(
        {userId:'testid'}, [noteOne._id, { title }]
      );
    ...
    it('should return zero for users that have none', function() {
      const res = Meteor.server.publish_handlers.notes.apply({userId: 'a guy with no notes'});
      const notes= res.fetch();
